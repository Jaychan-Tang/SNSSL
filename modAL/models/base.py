...
class BaseLearner(ABC, BaseEstimator):
    ...
    def decision_function(self, X: modALinput, **predict_proba_kwargs) -> Any:
        return self.estimator.decision_function(X, **predict_proba_kwargs)
    ...
    def query_svc(self, X_pool):
        query_result = self.query_strategy(self, X_pool)
        if isinstance(query_result, tuple):
            return query_result
        return query_result, retrieve_rows(X_pool, query_result)
        

    def query_with_density(self, X_pool, euclidean_density, *query_args, **query_kwargs) -> Union[Tuple, modALinput]:

        query_result = self.query_strategy(self, X_pool, euclidean_density=euclidean_density, *query_args, **query_kwargs)
    ...
    def query_with_edge(self, X_pool, predict_label, adjoin_sp,edge_weight=0.25, *query_args, **query_kwargs) -> Union[Tuple, modALinput]:
    
        query_result = self.query_strategy(self, X_pool, predict_label=predict_label, adjoin_sp=adjoin_sp, weight=edge_weight, *query_args,
                                           **query_kwargs)
    ...
    def query_with_adjoin(self, X_pool, adjoin_sp, adjoin_weight=0.1, *query_args, **query_kwargs) -> Union[
        Tuple, modALinput]:
    
        query_result = self.query_strategy(self, X_pool, adjoin_sp=adjoin_sp,
                                           *query_args,
                                           **query_kwargs)
    ...
    def query_with_prob(self, X_pool, *query_args, **query_kwargs) -> Union[
        Tuple, modALinput]:
...
class BaseCommittee(ABC, BaseEstimator):
    ...
    def query_with_weight(self, X_pool, weight, *query_args,  **query_kwargs) -> Union[Tuple, modALinput]:
        query_result = self.query_strategy(self, X=X_pool, weight=weight, *query_args, **query_kwargs)
